...
class _InquiryDetailScreenState extends State<InquiryDetailScreen> {
  ...
  // 🌟 삭제/화면 이탈은 _onDelete / _onEdit에서 직접 pop으로 처리하므로,
  //    여기서는 단순히 내용 갱신(재빌드)만 담당합니다.
  void _onChanged() {
    if (mounted) setState(() {});
  }
  ...
  Future<void> _onDelete() async {
    ...
    try {
      await _repo.delete(widget.inquiryId);
      if (!mounted) return;
        ...
        children: [
          ListView(
            ...
            children: [
              ...
              if (inquiry.contactEmail != null && inquiry.contactEmail!.isNotEmpty) ...[
                const SizedBox(height: 14),
                Text(
                  '답변받을 이메일  ${inquiry.contactEmail}',
                  style: const TextStyle(color: AppColors.textSecondary, fontSize: 13),
                ),
              ],
              if (answered && inquiry.adminMemo != null) ...[
