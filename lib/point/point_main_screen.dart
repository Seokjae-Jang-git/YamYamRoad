...
class _PointMainScreenState extends State<PointMainScreen> {
  ...
  Future<void> _purchaseEmoticon(EmoticonProduct emoticon) async {
    final confirmed = await PointShopDialogs.confirmPurchase(
      context: context,
  ...
  Future<void> _purchasePointPackage(PointPackage pointPackage) async {
    ...
    try {
      ...
      PointShopDialogs.showMessage(
        context,
        '${formatPointNumber(pointPackage.pointAmount)} 유료 포인트가 충전되었습니다. '
            '현재 유료 포인트는 ${formatPointNumber(result.remainingPaidPoint)} P입니다.',
      );
    } catch (error) {
      if (mounted) {
        PointShopDialogs.showMessage(
